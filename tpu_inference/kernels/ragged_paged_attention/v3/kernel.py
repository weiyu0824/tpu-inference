...
def _ragged_paged_attention_kernel_loop(
  ...
  def get_kv_cache_len_local(seq_idx):
    global_len = kv_lens_ref[seq_idx] - (
  ...
  def get_start_bkv_idx(seq_idx):
    ...
    start_idx = 0
    if sliding_window is not None:
      start_idx = jnp.maximum(local_cache_len - sliding_window, 0) // bkv_sz
    if skip_cache_attn:
    ...
    if not wait:
      ...
      if not skip_cache_attn:
        for i in range(bkv_p):
          ...
          _async_copy(
              cache_hbm_ref.at[
                  pl.ds(page_indices_ref[page_idx] * page_size, sz)
              ],
  ...
  def _update_kv_cache(
      seq_idx, bkv_sem_idx, offset, src_start_base, update_sz, *, wait=False
  ):
    if cp_group_size != None:
      _update_kv_cache_partial(
          seq_idx, bkv_sem_idx, offset, src_start_base, update_sz, wait=wait
      )
    else:
      _update_kv_cache_full(seq_idx, bkv_sem_idx, offset, update_sz, wait=wait)

  def _update_kv_cache_full(
      seq_idx, bkv_sem_idx, offset, update_sz, *, wait=False
  ):
  ...
  def _update_kv_cache_partial(
    ...
    if not wait:
      src_start = (
          src_start_base + local_offset_start * cp_group_size + cp_rank - offset
      )
      n_strided = bkv_sz // (
          cp_group_size if cp_group_size is not None else 1
      )  # static
  ...
  def start_update_kv_cache(
      seq_idx, bkv_sem_idx, offset, src_start_base, update_sz
  ):
  ...
  def wait_update_kv_cache(bkv_sem_idx):
    ...
    def _():
      ...
      _update_kv_cache(
          seq_idx, bkv_sem_idx, offset, src_start_base, update_sz, wait=True
      )
  ...
  def process(static_q_len=None):
    ...
    def get_next_bkv_ids(seq_idx, bq_idx, bkv_idx, bkv_sem_idx, *, num_bkv):
      ...
      if cp_group_size != None:
        ...
        next_bkv_idx = lax.select(
            is_last_bq, next_seq_start_bkv_idx, next_bkv_idx
        )
    ...
    def compute_with_bq(bq_idx):
      ...
      if skip_cache_attn:
        start_bkv_idx = jnp.maximum(start_bkv_idx, kv_cache_len_local // bkv_sz)
      if use_causal_mask:
      ...
      end_bkv_idx = jnp.maximum(
          cdiv(effective_kv_len, bkv_sz), start_bkv_idx + 1
      )
...
def get_default_block_sizes(
  ...
  num_q_heads_per_kv_head = next_power_of_2(
...
def ragged_paged_attention(
    ...
    distribution: jax.Array,  # i32[3]
    *,
    cp_rank: jax.Array | None = None,  # i32[1] - per-device rank, sharded along the DCP axis
    cp_group_size: int | None = None,
    use_causal_mask: bool = True,
  ...
  def run_rpa_kernel(
    ...
    in_specs = [
        ...
        ),  # lse_hbm (always present, aliased)
    ]
    ...
    q_out, kv_cache_out, lse_out = run(
        scalar_prefetches, q, kv, kv_cache, lse_hbm
    )
  ...
  if return_lse:
    ...
    lse = (
        lse_hbm[:, :, 0]
        .reshape(actual_num_kv_heads, max_num_tokens, num_q_heads_per_kv_head)
        .swapaxes(0, 1)[:, :, :actual_num_q_heads_per_kv_head]
